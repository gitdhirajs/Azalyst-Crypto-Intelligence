...
def log_fused_outcomes(previous_fused_signals: List[Dict]) -> None:
    """
    Match previous cycle's fused signals to latest features DataFrame
    to determine their outcomes, and write them to logs/fused_history.jsonl.
    """
    ...
    for signal in previous_fused_signals:
        ...
        mask = (features["symbol"] == symbol) & (
            (features["scan_time"] - sig_time).abs() <= timedelta(minutes=5)
        )
        ...
        entry = {
            ...
            "consensus_tier": signal.get("consensus_tier"),
