...
def compute_vol(prices: List[float]) -> float:
    ...
    mean_r=sum(returns)/len(returns); variance=sum((r-mean_r)**2 for r in returns)/len(returns)
...
class RiskManager:
    def __init__(self,max_positions:int=10,daily_loss_limit_pct:float=5.0,corr_threshold:float=0.75):
        self.max_positions=max_positions; self.daily_loss_limit=daily_loss_limit_pct/100.0; self.corr_threshold=corr_threshold; self.returns_cache={}
    def can_enter_position(self, symbol:str, direction:str, portfolio_value:float, open_positions:List[Dict], current_prices:Dict[str,float])->Tuple[bool,str]:
        if len(open_positions)>=self.max_positions:return False,f"Max positions ({self.max_positions}) reached."
        return True,""
    def update_returns_cache(self, symbol:str, daily_return:float):
        self.returns_cache.setdefault(symbol,[]).append(daily_return); self.returns_cache[symbol]=self.returns_cache[symbol][-30:]
    def check_correlation_with_existing(self, symbol:str, open_symbols:List[str])->Tuple[bool,float]:
        if symbol not in self.returns_cache or len(self.returns_cache[symbol])<5:return False,0.0
        max_corr=0.0
        for s in open_symbols:
            if s in self.returns_cache and len(self.returns_cache[s])>=5:
                ...
                if abs(c)>abs(max_corr): max_corr=c
