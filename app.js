...
function formatDate(value) {
  if (!value) return "n/a";
  const stamp = new Date(value);
  if (Number.isNaN(stamp.getTime())) return "n/a";
  return stamp.toLocaleString("en-IN", {
    year: "numeric", month: "short", day: "2-digit",
    hour: "2-digit", minute: "2-digit",
    timeZone: "Asia/Calcutta", timeZoneName: "short", hour12: false,
  });
}
...
function statusTone(status) {
  ...
  if (["healthy", "trained", "success", "completed"].includes(v)) return "good";
  if (["blocked", "failed", "error"].includes(v)) return "bad";
  if (["degraded", "no_data", "no_symbols", "queued", "in_progress", "requested"].includes(v)) return "warn";
  return "info";
}
...
async function fetchRepoJson(path, ref = "main") {
  ...
  try { return JSON.parse(decodeGitHubContent(response.content)); }
  catch { return null; }
}
...
function findLatestRun(latest, names) {
  for (const name of names) {
    if (latest.has(name)) return latest.get(name);
  }
  return null;
}
...
function mergePayloads(bootstrap, runtime) {
  ...
  const mainReport   = hasRuntime ? runtime?.main_report   || null : bootstrap?.main_report   || null;
  const hourlyReport = hasRuntime ? runtime?.hourly_report || null : bootstrap?.hourly_report || null;
  const hourlySignals = hasRuntime
    ? runtime?.hourly_live_signals || runtime?.hourly_report?.top_live_signals || []
    : bootstrap?.hourly_live_signals || bootstrap?.hourly_report?.top_live_signals || [];

  return {
    runtime_status: runtime?.runtime_status || bootstrap?.runtime_status || {},
    main_report: mainReport,
    hourly_report: hourlyReport,
    latest_scan_signals: hasRuntime ? runtime?.latest_scan_signals || [] : bootstrap?.latest_scan_signals || [],
    hourly_live_signals: hourlySignals,
    summary_markdown: runtime?.summary_markdown || bootstrap?.summary_markdown || "No summary published yet.",
    mainReportSource:   hasRuntime ? "runtime-data" : mainReport   ? "local bootstrap" : "missing",
    hourlyReportSource: hasRuntime ? "runtime-data" : hourlyReport ? "local bootstrap" : "missing",
  };
}
...
function renderTicker(repo, runtimeStatus, latestWorkflows) {
  ...
  const items = [
    ...
    `SYMBOLS ${scanner.symbols_scanned ?? "n/a"}`,
...
function renderModelCard(target, sourceTarget, report, sourceLabel, type) {
  ...
  const topFeature = report.top_features?.[0] || report.top_features_permutation?.[0];
  const extraLabel = type === "main"
  ...
  els.hourlySignalsTable.innerHTML = `
    <div class="signal-table">
      <div class="signal-head">
        <span>Symbol</span><span>Prob</span><span>RSI 1H</span><span>Body %</span><span>OI 1H %</span>
      </div>
      ${rows.join("")}
    </div>`;
}
...
function renderRuntimeFiles(files) {
  if (!files?.length) {
    els.runtimeFiles.innerHTML = `<div class="empty-state">No runtime report files visible yet.</div>`;
    return;
  }
...
function renderIssues(runtimeStatus, latestWorkflows, payload) {
  ...
  if (scanner.status === "blocked") {
    notes.unshift("GitHub-hosted runners are succeeding as jobs, but the market-data provider is rejecting data requests.");
  }
  if (payload.mainReportSource === "local bootstrap") {
    notes.push("Main model metrics shown from last local training snapshot — runner is not producing fresh market data.");
  }
  if (payload.hourlyReportSource === "local bootstrap") {
    notes.push("Hourly candle-pattern metrics also from local bootstrap snapshot.");
  }
  const deployRun = findLatestRun(latestWorkflows, ["pages build and deployment", "Deploy Dashboard"]);
  if (!deployRun) {
    notes.push("GitHub Pages deployment history not yet observed; site may still be bootstrapping.");
  }
...
function renderShell(repo, payload, runs, runtimeFiles, bootstrap, runtimePayload) {
  ...
  const scanner = runtimeStatus.scanner || {};
  const headline = scanner.headline || runtimeStatus.notes?.[0] || "Dashboard connected to repository telemetry.";
  ...
  renderModelCard(els.hourlyModelCard, els.hourlyModelSource, payload.hourly_report, payload.hourlyReportSource, "hourly");
